...
//댓글 답글
function fn_cmtReplyTo(bbs_Seq, cmt_Group)
{	
	if($(".btnCmtReply").css("display") == "block")
	{
		$(".btnCmtReply").css("display","none");
	...
	if($("#btnCmtReplyDone"+cmt_Group).css("display") == "block")
	{
		$("#btnCmtReplyDone"+cmt_Group).css("display","none");
	}
	else
	{
		$("#btnCmtReplyDone"+cmt_Group).css("display","block");
	}
	
	if($("#btnCmtReplyCancel"+cmt_Group).css("display") == "block")
	{
		$("#btnCmtReplyCancel"+cmt_Group).css("display","none");
	}
	else
	{
		$("#btnCmtReplyCancel"+cmt_Group).css("display","block");
	}
	
	
 	$('#replyLoc'+ cmt_Group).after("<tr id='xxxparent'><td/><td colspan='5'><input type='text' id='xxx' style='height:45px;'class='form-control mx-1' name='cmtContentxxx' placeholder='작성할 답글 내용을 입력하세요.'/></td></tr>");
	
}

function fn_replyDone(bbs_Seq, cmt_Group)
{	
	$.ajax({
		type: "POST",
		url: "/community/cmtWriteProc",
...
function fn_replyCancel(bbs_Seq, cmt_Group)
{
	$('#xxxparent').remove();
...
<div class="container">
	...
	<div class="row" style="margin-right:0; margin-left:0;">
		...
		<table class="table table-hover">
				...
				<c:if test="${!empty list}">
					<c:forEach var="commCmt" items="${list}" varStatus="status">
						...
						<c:if test="${commCmt.cmtIndent > 0}">
								...
								<div style="width:80%; height:25px; float:left;"><div id="cmtContentLoc${commCmt.cmtSeq}" style="display:block;">${commCmt.cmtContent}</div><div><input type="text" id="ttt${commCmt.cmtSeq}" style="display:none; height:25px;" class='form-control mx-1' placeholder="수정될 댓글내용을 입력하세요."/></div></div>
							</td>
							<td class="text-right">${commCmt.regDate}</td>
							<td class="text-right" id="updateLoc" width="200px;">
								<%-- <button type="button" id="btnCmtReply${commCmt.cmtGroup}" onClick="fn_cmtReplyTo(${commCmt.bbsSeq},${commCmt.cmtGroup});"><img src="/resources/images/reply.png" alt="답글이미지" height="10px"/></button> --%>
								<img id="btnCmtReply${commCmt.cmtSeq}" style="display:block; float:right; margin-left:10px;" class="btnCmtReply" onClick="fn_cmtReplyTo(${commCmt.bbsSeq},${commCmt.cmtGroup});" src="/resources/images/comm/reply.png" alt="답글이미지" title="답글" height="25px"/>&nbsp;&nbsp;
								<img id="btnCmtReplyCancel${commCmt.cmtSeq}" style="display:none; float:right; margin-left:5px;" onClick="fn_replyCancel(${commCmt.bbsSeq},${commCmt.cmtGroup});" src="/resources/images/comm/cancel.png" alt="답글작성취소이미지" title="답글작성취소" height="25px"/>&nbsp;&nbsp;
								<img id="btnCmtReplyDone${commCmt.cmtSeq}" style="display:none; float:right;" onClick="fn_replyDone(${commCmt.bbsSeq},${commCmt.cmtGroup});" src="/resources/images/comm/done.png" alt="답글작성완료이미지" title="답글작성완료" height="25px"/>&nbsp;&nbsp;								
			...
			<div>
			<div style="width:88%; float:left; margin-right:5px;"><input type="text" id="cmtContent" class='form-control mx-1' placeholder="새롭게 등록할 댓글내용을 입력하세요."/></div>
