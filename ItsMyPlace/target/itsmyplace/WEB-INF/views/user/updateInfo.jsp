...
function fn_userUpdate(subAreaNum){
      ...
      <div class="col-md-6 col-md-offset-3">
        <div class="block text-center">
          ...
          <form class="text-left clearfix" id="updateForm" method="post" action="index">          
			...
			시/도 &nbsp;&nbsp;
			&nbsp;&nbsp;&nbsp;
			<select name="country" id="country" onchange="fn_chng();">
				<option value="">전체</option>
<c:if test="${!empty subArea}">
   <c:forEach var="subAreaList" items="${subArea}" varStatus="status">
				<option value="${subAreaList.subAreaNum}" <c:if test="${subAreaList.subAreaNum eq user.subAreaNum}">selected</c:if>>${subAreaList.subAreaName}</option>
   </c:forEach>
</c:if>
			</select>         
			<br /><br />                 
