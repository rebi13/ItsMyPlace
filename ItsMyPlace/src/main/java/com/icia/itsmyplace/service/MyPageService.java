...
public class MyPageService {
	...
	public List<Point> myPointList(Point point){
		List<Point> pointList = null;
		try
		{
			pointList = myPageDao.myPointList(point);
		}
		catch(Exception e)
		{
			logger.error("[MyPageService] myPointList Exception", e);
		}
		return pointList;
	}
	
	public long userPointListCount(String userId) {
		long count = 0;
		
		try
		{
			count = myPageDao.userPointListCount(userId);
		}
		catch(Exception e)
		{
			logger.error("[MyPageService] userPointListCount Exception", e);
		}
